...
# ============================================================
n = 5
names = ["Купе нефирм", "Купе фирм", "Плацкарт нефирм", "Плацкарт фирм", "Сапсан"]

# Вместимость одного поезда (мест)
seats = np.array([180, 360, 486, 162, 554])   # для Сапсана ~600
# Частота (рейсов в день)
freq = np.array([50, 28, 50, 28, 30])
# Дневная пропускная способность
capacity = freq * seats

# Время в пути (часы)
tau = np.array([9, 7.5, 9.0, 7.5, 3.5])
# Комфорт (1..10)
alpha = np.array([8, 10, 4, 6, 8])
# Надёжность (0..1)
beta = np.array([0.9, 0.9, 0.9, 0.9, 0.95])

# Переменные издержки на пассажира (руб.)
a = np.array([550, 1400, 100, 400, 400])
# Переменные издержки на рейс (руб.)
b = np.array([490000, 650000, 490000, 650000, 950000])
# Постоянные издержки за день (руб.)
c_month = np.array([85000, 185000, 85000, 185000, 1430000])
c = c_month / 30
...
lambda3 = -0.8
lambda4 = 0.6
lam = 0.0006  # ценовая чувствительность
...
A_season = 0.25         # колебания ±25%
t0 = 180               # день пика (июль)
M = M_mean * (1 + A_season * np.sin(2 * np.pi * (days - t0/2) / 365))
